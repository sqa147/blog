#!/usr/bin/env node
// scripts/build-comments-side-by-side.js
//
// Finds the latest BEFORE / AFTER recordings produced by the
// "MiniBlog · Comments · before / after" describe block in
// tests/demo-before-after.spec.js, pads the shorter clip so both end
// together, then hstacks them into demo-output/comments-side-by-side.mp4.
//
...
const FINAL_OUT = path.join(OUTPUT_DIR, 'comments-side-by-side.mp4');
...
function findVideoByTestTitle(substr) {
  ...
  if (candidates.length === 0) {
    ...
    console.error('         Run:  npx playwright test tests/demo-before-after.spec.js -g "Comments"');
    process.exit(1);
  }
...
function main() {
  ...
  // Test directory names contain the describe + test title — match on title.
  // Playwright truncates the dir name, so we match the unique tail.
  const before = findVideoByTestTitle('post-detail-no-comments');
  const after = findVideoByTestTitle('post-detail-with-comments');
