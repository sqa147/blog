    ...
    # --------------------------------------------------------------------- M6
    {
        ...
        "scenarios": [
            ...
            ["TS-PDT-05", "Post Detail",
             ...
             "Validation, UI"],
            ["TS-PDT-06", "Post Detail",
             "Visitors and members can read comments under a post",
             "Verify that the comments section, count, and oldest-first list are shown on every post detail page.",
             "Positive, UI, Functional"],
            ["TS-PDT-07", "Post Detail",
             "Logged-in members can add a comment; visitors are prompted to log in",
             "Verify that the comment form appears for signed-in users and is replaced by a login prompt for visitors.",
             "Positive, Negative, UI"],
            ["TS-PDT-08", "Post Detail",
             "Empty or whitespace-only comments are not accepted",
             "Verify that the comment field is required and that whitespace-only content is rejected by the server.",
             "Negative, Validation"],
            ["TS-PDT-09", "Post Detail",
             "Only the comment's author can delete that comment",
             "Verify that the Delete button on a comment is shown only to its author and that other users cannot remove it.",
             "Validation, Negative, Security"],
            ["TS-PDT-10", "Post Detail",
             "Comment length stops at 2000 characters",
             "Verify the 2000-character comment boundary and that 2001 is rejected.",
             "Boundary, Validation"],
            ["TS-PDT-11", "Post Detail",
             "Comments containing HTML-like text are shown safely",
             "Verify that script-like / image-like text inside a comment is rendered as plain readable text.",
             "Validation, UI, Security"],
        ],
        "cases": [
            ...
            # ── Comments feature (this PR) ──────────────────────────────
            ["TC-PDT-11", "TS-PDT-06", "Post Detail",
             "Comments section is visible with empty-state text when a post has no comments",
             "High", "Smoke", "Positive",
             "A post exists with no comments.",
             "An existing post.",
             "1. Open the post's detail page.\n2. Scroll to the area below the post.",
             "A \"Comments (0)\" heading is shown. Below it the message \"No comments yet. Be the first!\" is displayed and no comment cards appear."],

            ["TC-PDT-12", "TS-PDT-07", "Post Detail",
             "Visitor sees a login prompt instead of the comment form",
             "High", "Smoke", "Negative",
             "The user is not logged in.",
             "An existing post.",
             "1. Make sure no user is logged in.\n2. Open any post's detail page.\n3. Look at the comments area.",
             "A panel reading \"Log in to leave a comment.\" with a Log in link is shown. The comment text box and Post comment button are not displayed."],

            ["TC-PDT-13", "TS-PDT-07", "Post Detail",
             "Logged-in user can post a comment that appears immediately under the post",
             "High", "Smoke", "Positive",
             "The user is logged in. A post exists.",
             "Comment text: \"Great post! Thanks for sharing.\"",
             "1. Open a post's detail page while logged in.\n2. Type the comment text into the text box.\n3. Click Post comment.",
             "The new comment appears in the list with the user's name and the current date. The counter updates to \"Comments (1)\". The text box is cleared and ready for another comment."],

            ["TC-PDT-14", "TS-PDT-08", "Post Detail",
             "Empty comment cannot be submitted from the page",
             "Medium", "Regression", "Negative",
             "The user is logged in.",
             "Leave the comment text box blank.",
             "1. Open a post's detail page.\n2. Click Post comment without typing anything.",
             "The form does not submit. The text box shows a \"please fill out this field\" prompt and the comment count does not change."],

            ["TC-PDT-15", "TS-PDT-08", "Post Detail",
             "Whitespace-only comment is rejected with a clear message",
             "Medium", "Regression", "Validation",
             "The user is logged in.",
             "Comment text: \"     \" (only spaces and newlines)",
             "1. Submit a comment containing only spaces and newlines.",
             "The validation message \"content is required\" is displayed and no comment is added to the post."],

            ["TC-PDT-16", "TS-PDT-06", "Post Detail",
             "Comments are shown oldest-first and the counter reflects the total",
             "Medium", "Regression", "Functional",
             "A post has three comments added in order: \"first\", \"second\", \"third\".",
             "Comments: first, second, third",
             "1. Open the post's detail page.\n2. Look at the order of comments and the counter.",
             "The counter reads \"Comments (3)\". The comments appear top to bottom as \"first\", \"second\", \"third\"."],

            ["TC-PDT-17", "TS-PDT-09", "Post Detail",
             "Author of a comment can delete it after confirming",
             "High", "Smoke", "Positive",
             "The user has just posted a comment on a post.",
             "Comment text: \"mine to remove\"",
             "1. On the post's detail page, click Delete on the user's own comment.\n2. Confirm the \"Delete this comment?\" prompt.",
             "The comment is removed from the list. The counter updates to \"Comments (0)\" and the empty-state message reappears."],

            ["TC-PDT-18", "TS-PDT-09", "Post Detail",
             "User cannot see a Delete button on someone else's comment",
             "High", "Regression", "Negative",
             "User Sara is logged in. The comment was written by Ali Khan.",
             "Other user: sara.n / sara.n@gmail.com\nComment author: ali.khan",
             "1. Log in as Sara.\n2. Open the post's detail page.\n3. Look at Ali Khan's comment.",
             "The comment is visible with the author's name, but no Delete button is shown on it."],

            ["TC-PDT-19", "TS-PDT-09", "Post Detail",
             "Cross-user delete request is rejected by the platform",
             "High", "Regression", "Security",
             "User Sara has a valid session. The comment belongs to Ali Khan.",
             "Acting user: sara.n\nComment owner: ali.khan",
             "1. While logged in as Sara, attempt to delete one of Ali Khan's comments.",
             "The request is rejected with the message \"You can only delete your own comments\". The comment continues to appear under the post."],

            ["TC-PDT-20", "TS-PDT-11", "Post Detail",
             "Comment containing HTML-like text is shown as plain text",
             "High", "Regression", "Validation",
             "A comment has been added with script-like text (input-sanitization check).",
             "Comment text: <img src=x onerror=alert(1)> & <script>alert(2)</script>",
             "1. Open the post's detail page.\n2. Look at the comment.",
             "The comment is shown as plain readable text. No image is loaded, no script runs, and no popup appears."],

            ["TC-PDT-21", "TS-PDT-10", "Post Detail",
             "Comment of exactly 2000 characters is accepted",
             "High", "Regression", "Boundary",
             "The user is logged in.",
             "Comment text: 2000 letters of \"a\".",
             "1. Submit a comment with 2000 characters of content.",
             "The comment is created successfully. The full 2000 characters are saved and shown in the comment list."],

            ["TC-PDT-22", "TS-PDT-10", "Post Detail",
             "Comment longer than 2000 characters is rejected",
             "High", "Regression", "Boundary",
             "The user is logged in.",
             "Comment text: 2001 letters of \"a\".",
             "1. Submit a comment with 2001 characters of content.",
             "The validation message \"comment too long (max 2000 chars)\" is displayed and the comment is not saved."],

            ["TC-PDT-23", "TS-PDT-07", "Post Detail",
             "Posting a comment without a valid session is rejected",
             "High", "Regression", "Negative",
             "The user is not signed in.",
             "Comment text: \"try\"",
             "1. Without being signed in, attempt to post a comment to an existing post.",
             "The action is rejected with the message \"Missing token\" and no comment is added."],

            ["TC-PDT-24", "TS-PDT-06", "Post Detail",
             "Posting a comment to a non-existent post returns a clear not-found message",
             "Medium", "Regression", "Edge",
             "The user is logged in. The chosen post does not exist.",
             "Post reference: 999999\nComment text: \"ghost\"",
             "1. While logged in, attempt to post a comment on the non-existent post.",
             "The validation message \"Post not found\" is displayed and no comment is created."],
        ],
    },
