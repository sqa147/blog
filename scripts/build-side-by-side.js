#!/usr/bin/env node
// scripts/build-side-by-side.js
//
// Finds the latest BEFORE / AFTER recordings produced by
// tests/demo-before-after.spec.js, pads the shorter clip so both end together,
// then hstacks them into demo-output/remember-me-side-by-side.mp4.
//
// The on-screen "BEFORE" / "AFTER" banner is already baked into each take
// (via the test's installHeadingBanner) so no ffmpeg drawtext overlay is
// needed — which side-steps the Windows fontconfig dependency.
...
const FINAL_OUT = path.join(OUTPUT_DIR, 'remember-me-side-by-side.mp4');
...
function findVideoByTestTitle(substr) {
  ...
  if (candidates.length === 0) {
    ...
    console.error('         Run:  npx playwright test tests/demo-before-after.spec.js');
    process.exit(1);
  }
...
function main() {
  ...
  const before = findVideoByTestTitle('old-login');
  const after = findVideoByTestTitle('new-login');
  ...
  // tpad freezes each clip's last frame until both end at `target`, then
  // hstack composes them side-by-side. Banners are baked in already.
  const filter = [
